...
@router.post("/{pago_id}/revertir-pago-completo")
async def revertir_pago_completo(
    pago_id: uuid.UUID,
    request: Request,
    ...
    """
    ENDPOINT TEMPORAL — Revierte un pago completo (pagado=True) restaurando
    saldos del crédito y eliminando cuotas posteriores generadas tras este pago.

    Solo aplica si NO hay cuotas posteriores con pago real (capital_pagado > 0
    o interes_pagado > 0). En ese caso devuelve 422.
    """
    ...
    if not pago.pagado:
        raise HTTPException(status_code=422, detail="El pago no estaba marcado como pagado")
    ...
    for p in posteriores:
        if p.capital_pagado > 0 or p.interes_pagado > 0 or p.pagado:
            raise HTTPException(
                status_code=422,
                detail=(
                    f"Cuota posterior #{p.numero_cuota} tiene actividad — "
                    "no se puede revertir automáticamente."
                ),
            )

    # Calcular deltas a devolver al crédito (debe reflejar exactamente lo que
    # el pago descontó en su momento — ver _pago_exacto / _pago_parcial /
    # confirmar_excedente en pago_service.py).
    from app.models.pago import DestinoExcedente
    excedente = pago.capital_pagado + pago.interes_pagado - pago.monto_a_pagar
    if excedente < 0:
        excedente = Decimal("0.00")

    if pago.es_excedente_a is None or excedente == 0:
        delta_capital = pago.capital_pagado
        delta_intereses = pago.interes_pagado
    elif pago.es_excedente_a == DestinoExcedente.capital:
        delta_capital = pago.capital_a_pagar + excedente
        delta_intereses = pago.interes_a_pagar
    else:  # DestinoExcedente.intereses
        delta_capital = pago.capital_a_pagar
        delta_intereses = pago.interes_a_pagar + excedente

    snapshot_pago_antes = {
        "pagado": "True",
        "capital_pagado": str(pago.capital_pagado),
        "interes_pagado": str(pago.interes_pagado),
        "validado_recaudador": str(pago.validado_recaudador),
        "es_excedente_a": str(pago.es_excedente_a) if pago.es_excedente_a else "None",
        "tipo_validacion": str(pago.tipo_validacion) if pago.tipo_validacion else "None",
    }
    snapshot_credito_antes = {
        "saldo_capital": str(credito.saldo_capital),
        "saldo_intereses": str(credito.saldo_intereses),
        "activo": str(credito.activo),
    }

    # Soft-delete cuotas posteriores (todas sin actividad)
    ahora = ahora_bogota()
    posteriores_ids: list = []
    for p in posteriores:
        p.deleted_at = ahora
        posteriores_ids.append(str(p.id))

    # Resetear el pago a "pendiente sin check"
    pago.pagado = False
    pago.validado_recaudador = False
    pago.capital_pagado = Decimal("0.00")
    pago.interes_pagado = Decimal("0.00")
    pago.fecha_pago_real = None
    pago.es_excedente_a = None
    pago.tipo_validacion = None

    # Restaurar saldos del crédito
    credito.saldo_capital = credito.saldo_capital + delta_capital
    credito.saldo_intereses = credito.saldo_intereses + delta_intereses

    # Reactivar si estaba cerrado por este pago
    if not credito.activo:
        credito.activo = True
    ...
    await audit_service.registrar_actualizacion_campos(
        ...
        cambios={
            "pagado": (snapshot_pago_antes["pagado"], "False"),
            "capital_pagado": (snapshot_pago_antes["capital_pagado"], "0.00"),
            "interes_pagado": (snapshot_pago_antes["interes_pagado"], "0.00"),
            "validado_recaudador": (snapshot_pago_antes["validado_recaudador"], "False"),
            "es_excedente_a": (snapshot_pago_antes["es_excedente_a"], "None"),
            "tipo_validacion": (snapshot_pago_antes["tipo_validacion"], "None"),
        },
    )
    await audit_service.registrar_actualizacion_campos(
        ...
        usuario_id=current_user.id, ip_origen=ip,
        cambios={
            "saldo_capital": (snapshot_credito_antes["saldo_capital"], str(credito.saldo_capital)),
            "saldo_intereses": (snapshot_credito_antes["saldo_intereses"], str(credito.saldo_intereses)),
            "activo": (snapshot_credito_antes["activo"], str(credito.activo)),
        },
    )
    for pid in posteriores_ids:
        await audit_service.registrar_eliminacion(
            db=db, entidad="pagos", entidad_id=uuid.UUID(pid),
            usuario_id=current_user.id, ip_origen=ip,
        )

    return {
        "pago_revertido": str(pago.id),
        "cuotas_posteriores_eliminadas": posteriores_ids,
        "credito": {
            "id": str(credito.id),
            "saldo_capital_anterior": snapshot_credito_antes["saldo_capital"],
            "saldo_capital_nuevo": str(credito.saldo_capital),
            "saldo_intereses_anterior": snapshot_credito_antes["saldo_intereses"],
            "saldo_intereses_nuevo": str(credito.saldo_intereses),
        },
    }
