...
> Change `reportes-cartera-vencida-y-rango-fechas` — Phase 2 items 13+14 (fused).
> Capability: `GET /reportes/ingresos` (existing structure, generalized with interval
> filter support) and `GET /reportes/cartera-vencida` (new, event-based on
> `fecha_entrada_mora` from the `overdue-evaluation` capability). Both support two
> mutually exclusive filter modes — "por momento" (`anio`/`mes`/`momento`) or
> "por intervalo" (`fecha_desde`/`fecha_hasta`). Cartera Vencida is live-recomputed,
> never a historical snapshot. All dates below are Bogotá dates.
