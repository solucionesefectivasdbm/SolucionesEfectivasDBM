...
| Inheritance (decision 1) | `cuenta_heredable(repartos)` returns `pago.cuenta_bancaria_id` unchanged when the reparto has exactly 1 recipient row, or `None` when it has 2 or more rows (any mix of cuenta and cliente recipients) | Distinct-cuenta counting; pick the largest share | Owner decision (confirmed 2026-09-22): today's inheritance behavior is untouched for single-recipient payments — it only breaks when a payment was split among **more than one recipient**, regardless of recipient type |
...
- [x] Inheritance criterion. **Owner decision**: today's inheritance (`generar_siguiente_cuota` carrying `pago.cuenta_bancaria_id` forward) is unchanged for single-recipient payments. It breaks (next cuota gets `None`) when the reparto has **more than one recipient row, of any type** — cuenta A + cliente X still counts as 2 recipients and blocks inheritance, same as cuenta A + cuenta B. See the corrected "Inheritance" row in Key Decisions above.
