...
(`pagado = false` and `fecha_maxima < today`) wins over the deferred style;
otherwise deferred style. There MUST be no escalated style for `>= 2`; only
the badge number changes. The `/pagos/aplazados` page MUST default to
pending-only and offer a control to include paid rows.

#### Scenario: Deferred not overdue `[manual]`

- GIVEN a pending row deferred to a future date
- THEN it renders with the deferred style and badge `1`

#### Scenario: Deferred and overdue again `[manual]`

- GIVEN a row deferred to a date now in the past
- THEN it renders red with the badge still visible
